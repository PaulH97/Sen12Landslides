...
class FPNConvLSTM(nn.Module):
    def __init__(
        ...
        if self.mid_conv:
            dim = self.n_channels * self.n_levels // 2
        ...
        self.outconv = nn.Conv2d(
            in_channels=self.hidden_size, out_channels=self.num_classes, kernel_size=1
        )
    ...
    def forward(self, input, batch_positions=None):
        ...
        out = self.inconv_block.smart_forward(input)
        out = self.pyramid.smart_forward(out)
        if self.mid_conv_block is not None:
            out = self.mid_conv_block.smart_forward(out)
